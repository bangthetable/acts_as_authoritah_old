...
1. Create an excel sheet (or download a sample from here and modify) and save it at config/acl/default.xls (create the folder first time)
   When the post is unpublished, there are two rules which are different from the default set of rules. We need to add these two rules alone in config/acl/unpublished.xls (download here).
